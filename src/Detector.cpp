#define NOMINMAX //this prevents windows.h from breaking std::max
...
std::shared_ptr<Mat> Detector::getImageRoiInGreyScale(std::shared_ptr<Mat> img) {
	auto temp = *img.get();
...
std::vector<std::vector<Point>> Detector::detectBolts(std::shared_ptr<Mat> roi, int threshold) {
...
std::vector<cv::Point2i> Detector::detect(std::shared_ptr<cv::Mat> img)
{
	auto roiGrey = getImageRoiInGreyScale(img);
	return detectBestBolts(roiGrey);
}

void Detector::determineOptimalThreshold(std::shared_ptr<Mat> roi)
{
